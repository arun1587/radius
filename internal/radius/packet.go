...
func (p *Packet) Encode() (b []byte, err error) {
	p = p.Copy()
	/*for now commenting the Msg_Authenticator: Arun
	p.SetAVP(AVP{
...
func (p *Packet) checkMessageAuthenticator() (err error) {
	...
	fmt.Println("AuthenticatorValue == ", AuthenticatorValue)
	defer func() {
		Authenticator.Value = AuthenticatorValue
		fmt.Println("AuthenticatorValue in defer == ", AuthenticatorValue)
	}()
	...
	fmt.Println("AuthenticatorValue after defer == ", AuthenticatorValue)
	return nil
}
...
func (p *Packet) GetLoraJoinRequest() (*lorawan.PHYPayload, error) {
	avp := p.GetAVP(LoraJoinRequest)
	if avp == nil {
		return nil, errors.New("LoraJoinRequest AVP not found")
	}

	jR := &lorawan.PHYPayload{}
	if err := jR.UnmarshalBinary(avp.Value); err != nil {
		return nil, err
	}
...
func (p *Packet) GetLoraJoinAccept() (*lorawan.PHYPayload, error) {
	avp := p.GetAVP(LoraJoinAccept)
	if avp == nil {
		return nil, errors.New("LoraJoinAccept AVP not found")
	}

	jA := &lorawan.PHYPayload{}
	if err := jA.UnmarshalBinary(avp.Value); err != nil {
		return nil, err
	}
